...
// ✅ Safe supabase URL fallback (prevents prod crashes when SUPABASE_URL isn't set)
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseKey) {
      return NextResponse.json(
        { ok: false, error: "Server missing Supabase config", details: { supabaseUrl: !!supabaseUrl, serviceKey: !!supabaseKey } },
        { status: 500 }
      );
    }
    ...
    if (!res.ok) {
      return NextResponse.json(
        { ok: false, error: "Square availability failed", details: json },
        { status: 500 }
      );
    }

    // ✅ Provide a compact "slots" helper for VAPI (does not break existing "availability" field)
    const availabilities = Array.isArray((json as any)?.availabilities) ? (json as any).availabilities : [];
    const slots = availabilities.slice(0, 12).map((a: any) => ({
      start_at: a?.start_at,
    ...
    console.error("squareAvailability crashed:", e);
    return NextResponse.json(
