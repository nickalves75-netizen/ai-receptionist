...
export default function PortalPage() {
  ...
  async function load() {
    ...
    if (!mRes.ok) {
      setLoading(false);
      setErr("Could not load metrics. Please log in again.");
      return;
    }
  ...
  return (
    <main style={{ padding: 24, maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(6, minmax(0, 1fr))", gap: 12, marginTop: 18 }}>
        ...
        <Card
          label="Avg Duration"
          value={metrics ? fmtSec(metrics.avg_duration_seconds) : undefined}
          loading={loading}
        />
      </div>
      ...
      <div style={{ marginTop: 18, ...panel() }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
          <div>
            <div style={{ fontWeight: 700 }}>Recent Calls</div>
        ...
        <div style={{ marginTop: 12, overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={6} style={{ padding: 14, opacity: 0.7 }}>Loading…</td></tr>
              ) : rows.length ? (
                rows.map((r) => (
                ...
                <tr><td colSpan={6} style={{ padding: 14, opacity: 0.7 }}>No calls yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
